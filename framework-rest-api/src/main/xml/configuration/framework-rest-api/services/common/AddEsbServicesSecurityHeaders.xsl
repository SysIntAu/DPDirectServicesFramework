...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:dp="http://www.datapower.com/extensions"
	...
	<xsl:variable name="API_PROXY_ID">
		<xsl:choose>
	...
	<xsl:variable name="API_SERVICE_REQUEST_ACTION">
		<xsl:variable name="API_INTERFACE_NS_PREFIX" select="'http://www.dpdirect.org/Namespace/'"/>
		<xsl:variable name="API_INTERFACE_NS_SUFFIX" select="'/Services/Interface/V1/'"/>
		<xsl:variable name="API_INTERFACE_PORT_SUFFIX" select="'_PortType_V1/'"/>
		<xsl:if test="normalize-space($API_PROXY_ID) != ''">
			<xsl:value-of select="concat($API_INTERFACE_NS_PREFIX, $API_PROXY_ID, $API_INTERFACE_NS_SUFFIX,  $API_PROXY_ID, $API_INTERFACE_PORT_SUFFIX, $REQUEST_ACTION_ENDPOINT)"/>
		</xsl:if>
	</xsl:variable>
	...
	<xsl:template match="soap:Header">
		<xsl:copy>
			...
			<xsl:if test="not(wsa:Action)">
				<wsa:Action><xsl:value-of select="$API_SERVICE_REQUEST_ACTION"/></wsa:Action>
			</xsl:if>
	...
	<xsl:template match="wsa:Action">
		<xsl:copy>
			<xsl:choose>
				...
				<xsl:when test="normalize-space($API_SERVICE_REQUEST_ACTION) != ''">
					<xsl:value-of select="$API_SERVICE_REQUEST_ACTION"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="."/>
				</xsl:otherwise>
